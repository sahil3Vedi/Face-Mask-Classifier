...
IMAGE_DIR = 'data/mask_images'
GRAY_IMAGE_PATH = os.path.join(IMAGE_DIR, 'gray-mask.png')
...
def create_mask(image_path):
    pic_path = image_path
    mask_path_1 = BLUE_IMAGE_PATH
    mask_path_2 = GRAY_IMAGE_PATH
    show = False
    model = "hog"
    FaceMasker(pic_path, mask_path_1, show, model).mask()
    FaceMasker(pic_path, mask_path_2, show, model).mask()
...
class FaceMasker:
    ...
    def _save(self):
        ...
        new_file_name = path_splits[0].split("\\")[-1]
        if self.mask_path == BLUE_IMAGE_PATH:
            new_face_path = 'data/with_mask_blue/' + new_file_name + '-with-blue-mask' + path_splits[1]
        if self.mask_path == GRAY_IMAGE_PATH:
            new_face_path = 'data/with_mask_gray/' + new_file_name + '-with-gray-mask' + path_splits[1]
